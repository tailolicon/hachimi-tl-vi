...
def harden(repo_root: Path = ROOT) -> bool:
    ...
    for term in terms:
        ...
        term["context_note"] = (
            "快人一步 is a Skill title only when it is the complete source string. In longer prose such as 速度快人一步 it is an ordinary phrase meaning to be a step ahead."
        )
        break
    if not matched:
...
def main() -> int:
    changed = harden(ROOT)
    print(f"fast_step_skill_context_hardening_changed={str(changed).lower()}")
    return 0
