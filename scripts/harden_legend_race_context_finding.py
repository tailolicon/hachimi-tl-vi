...
def harden(repo_root: Path = ROOT) -> bool:
    ...
    for term in terms:
        ...
        term["context_note"] = (
            "Generic 比赛/レース maps to Cuộc đua, but must not fire inside established named-mode compounds such as 传奇比赛 (Legend Race)."
        )
        break
    if not matched:
...
def main() -> int:
    changed = harden(ROOT)
    print(f"legend_race_context_hardening_changed={str(changed).lower()}")
    return 0
