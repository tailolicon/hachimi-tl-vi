...
"""Prevent Mainichi Hai's short zh-CN alias from overmatching Daily Hai Junior Stakes."""
...
def harden(repo_root: Path = ROOT) -> bool:
    ...
    for term in terms:
        ...
        guard_note = (
            "Context guard: the short Mainichi Hai alias 每日杯 must not match inside the distinct full race "
            "每日杯新马锦标 / Daily Hai Junior Stakes."
        )
...
def main() -> int:
    print(f"mainichi_hai_context_hardening_changed={str(harden(ROOT)).lower()}")
    return 0
