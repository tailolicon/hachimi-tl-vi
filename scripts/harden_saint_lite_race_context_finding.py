...
"""Prevent the Saint Lite character alias from overmatching the St. Lite Kinen race name."""
...
def harden(repo_root: Path = ROOT) -> bool:
    ...
    for term in terms:
        ...
        guard_note = (
            "Context guard: the Saint Lite character alias 圣列特 must not match inside the full race "
            "name 圣列特纪念 / St. Lite Kinen; the locked race identity is authoritative there."
        )
...
def main() -> int:
    print(f"saint_lite_race_context_hardening_changed={str(harden(ROOT)).lower()}")
    return 0
