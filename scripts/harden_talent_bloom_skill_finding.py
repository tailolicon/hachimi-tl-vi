...
def harden(repo_root: Path = ROOT) -> bool:
    ...
    for term in terms:
        ...
        term["context_note"] = (
            "The zh-CN alias 开花 is a Skill title only when it is the complete source string. "
            "Do not match progression/system phrases such as 才能开花, where 开花 belongs to the talent/star-ascension mechanic rather than the Skill title."
        )
        break
    if not matched:
...
def main() -> int:
    changed = harden(ROOT)
    print(f"talent_bloom_skill_hardening_changed={str(changed).lower()}")
    return 0
