...
def harden(repo_root: Path = REPO_ROOT) -> None:
    ...
    _upsert(
        community_terms,
        {
            "id": "common.stat_cap.singlemode400100",
            "source_aliases": ["{0}的上限已<color=#FF6D26>达到极限</color>"],
            "preferred": "Stat Cap",
            "accepted": ["Stat Cap"],
            "compact": [],
            "forbidden": ["giới hạn tối đa", "Giới hạn tối đa"],
            "require_accepted": True,
            "source_paths": ["localize_dict.json"],
            "key_exact": ["SingleMode400100"],
            "match_mode": "exact",
            "invalidation_scope": "item",
            "basis": (
                "Career stat-limit status UI. The established player term is Stat Cap; exact-key scope avoids "
                "matching generic prose about limits or maximum values."
            ),
        },
    )
    _upsert(
        community_terms,
        {
            "id": "common.stat_cap.singlemode400102",
            "source_aliases": ["{0}的上限"],
            "preferred": "Stat Cap",
            "accepted": ["Stat Cap"],
            "compact": [],
            "forbidden": ["Giới hạn của", "giới hạn của"],
            "require_accepted": True,
            "source_paths": ["localize_dict.json"],
            "key_exact": ["SingleMode400102"],
            "match_mode": "exact",
            "invalidation_scope": "item",
            "basis": (
                "Career stat-cap label. Stat Cap is the established player-facing term; exact-key scope prevents "
                "ordinary limit wording elsewhere from becoming canonical."
            ),
        },
    )
    _upsert(
        community_terms,
        {
            "id": "common.max_energy.singlemode400103",
            "source_aliases": ["体力最大值"],
            "preferred": "Max Energy",
            "accepted": ["Max Energy"],
            "compact": [],
            "forbidden": ["Thể lực tối đa", "thể lực tối đa"],
            "require_accepted": True,
            "source_paths": ["localize_dict.json"],
            "key_exact": ["SingleMode400103"],
            "match_mode": "exact",
            "invalidation_scope": "item",
            "basis": (
                "Career maximum-Energy status label. Energy is already canonical in this domain; exact-key scope "
                "keeps generic physical-stamina prose outside the rule."
            ),
        },
    )
