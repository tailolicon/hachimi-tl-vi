...
def harden(repo_root: Path = ROOT) -> bool:
    ...
    for term in terms:
        ...
        term["context_note"] = (
            "Generic 比赛/レース maps to Cuộc đua, but must not fire when 比赛 denotes non-racing sports competitions; "
            "the U.A.F. scenario phrase 15种独特的比赛 refers to its fifteen sports disciplines."
        )
        break
    if not matched:
...
def main() -> int:
    changed = harden(ROOT)
    print(f"uaf_competition_context_hardening_changed={str(changed).lower()}")
    return 0
