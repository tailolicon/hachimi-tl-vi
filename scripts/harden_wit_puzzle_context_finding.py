...
"""Guard the Wit stat alias 智力 from ordinary 智力扣 puzzle/toy compounds."""
...
def harden(repo_root: Path = ROOT) -> bool:
    ...
    for term in terms:
        ...
        term["basis"] = (
            "Common EN-version Wit stat label. The zh-CN alias 智力 is context-sensitive and must not "
            "canonicalize ordinary compounds such as 智力扣 (a disentanglement puzzle/toy)."
        )
        break
    if not matched:
...
def main() -> int:
    changed = harden(ROOT)
    print(f"wit_puzzle_context_hardening_changed={str(changed).lower()}")
    return 0
