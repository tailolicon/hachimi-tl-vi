...
from scripts.harden_abyss_terpsichore_finding import DECISION, PREFERRED, SOURCE_JA, SOURCE_ZH, TERM_ID, harden

FINDING_ID = "cf-9ba99786ace9b115"
...
def test_abyss_terpsichore_resolves_live_skill_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Khiêu vũ nơi vực sâu" in rule["forbidden"]
...
def test_abyss_terpsichore_rule_is_category_and_path_scoped(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    wrong_category = refresh_canonical_resolutions(
        tmp_path, {"schema_version": 1, "findings": [_finding(prefix=[["144"]])]}
    )["findings"][0]
