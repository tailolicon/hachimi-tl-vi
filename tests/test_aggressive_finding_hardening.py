...
from scripts.harden_aggressive_finding import (
    DECISION_ID,
    FINDING_ID,
    RULE_ID,
    SOURCE_JA,
...
def test_hardener_resolves_aggressive_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Lấy công làm thủ" in rule["forbidden"]
