...
from scripts.harden_agnes_digital_final_sprint_finding import DECISION, FINDING_ID, RULE, harden
...
def test_agnes_digital_final_sprint_resolves_in_skill_category(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert rule["preferred"] == "OMG! (ﾟ∀ﾟ) The Final Sprint! ☆"
    ...
    assert decision["target_vi"] == "OMG! (ﾟ∀ﾟ) The Final Sprint! ☆"
    assert decision["ja"] == ["尊み☆ﾗｽﾄｽﾊﾟ—(ﾟ∀ﾟ)—ﾄ!"]
    ...
    assert finding["review_resolution"] == {
        ...
        "action": "lock",
        "target_vi": "OMG! (ﾟ∀ﾟ) The Final Sprint! ☆",
    }
    assert finding["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.agnes_digital.omg_final_sprint",
        "target_vi": "OMG! (ﾟ∀ﾟ) The Final Sprint! ☆",
    }


def test_agnes_digital_final_sprint_does_not_canonically_match_outside_category(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True

    ledger = {"schema_version": 1, "findings": [_finding("16")]}
    finding = refresh_canonical_resolutions(tmp_path, ledger)["findings"][0]
    assert finding["review_resolution"]["target_vi"] == "OMG! (ﾟ∀ﾟ) The Final Sprint! ☆"
