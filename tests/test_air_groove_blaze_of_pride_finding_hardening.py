...
from scripts.harden_air_groove_blaze_of_pride_finding import (
    DECISION_ID,
    FINDING_ID,
    RULE_ID,
    SOURCE_JA,
...
def test_hardener_resolves_blaze_of_pride_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Lưỡi đao vinh quang" in rule["forbidden"]
...
def test_exact_rule_does_not_cover_longer_text_or_other_file(tmp_path: Path) -> None:
    ...
    longer = _finding(source=f"获得「{SOURCE_ZH}」技能灵感")
