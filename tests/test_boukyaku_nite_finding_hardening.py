...
from scripts.harden_boukyaku_nite_finding import (
    DECISION_ID,
    FINDING_ID,
    RULE_ID,
    SOURCE,
...
def test_hardener_resolves_boukyaku_nite_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Trong lãng quên" in rule["forbidden"]
