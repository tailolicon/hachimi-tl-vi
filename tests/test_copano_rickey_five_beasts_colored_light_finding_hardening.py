...
from scripts.harden_copano_rickey_five_beasts_colored_light_finding import (
...
def test_five_beasts_skill_resolves_live_unscoped_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    rule = next(
        item for item in community["terms"] if item["id"] == FIVE_BEASTS_COLORED_LIGHT["id"]
    )
    assert rule["preferred"] == PREFERRED
    assert rule["source_paths"] == ["text_data_dict.json"]
    assert rule["match_mode"] == "exact"
    assert "Hợp tấu ánh sắc của Ngũ Hành Thú" in rule["forbidden"]
    ...
    decision = next(
        ...
        if item["decision_id"] == FIVE_BEASTS_COLORED_LIGHT_DECISION["decision_id"]
    )
    assert decision["target_vi"] == PREFERRED
    assert decision["ja"] == ["五獣挙りて彩光奏づ"]
    ...
    assert finding["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.copano_rickey.five_beasts_colored_light",
        "target_vi": PREFERRED,
    }


def test_five_beasts_rule_does_not_overmatch_longer_source(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
...
def test_five_beasts_rule_does_not_resolve_other_source_path(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
