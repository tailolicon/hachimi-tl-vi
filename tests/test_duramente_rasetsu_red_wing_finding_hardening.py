...
from scripts.harden_duramente_rasetsu_red_wing_finding import (
    DECISION_ID,
    FINDING_ID,
    RULE_ID,
    SOURCE_JA,
...
def test_hardener_resolves_duramente_unique_skill_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Xích Dực La Sát vượt cửu thiên" in rule["forbidden"]
