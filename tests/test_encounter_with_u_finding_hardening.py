...
from scripts.harden_encounter_with_u_finding import (
    DECISION_ID,
    FINDING_ID,
    RULE_ID,
    SOURCE_JA,
...
def _finding(*, prefix: str = "172", source_path: str = "text_data_dict.json") -> dict:
    return {
...
def test_hardener_resolves_encounter_with_u_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Kỳ ngộ nhờ duyên U" in rule["forbidden"]
...
def test_scoped_contains_rule_does_not_cover_other_categories_or_files(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True

    other_category = _finding(prefix="147")
    other_category["suggested_targets_vi"] = [TARGET]
    other_resolved = refresh_canonical_resolutions(tmp_path, {"schema_version": 1, "findings": [other_category]})["findings"][0]
    assert other_resolved["canonical_resolution"] is None
