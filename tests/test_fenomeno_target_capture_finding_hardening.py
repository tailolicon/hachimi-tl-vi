...
from scripts.harden_fenomeno_target_capture_finding import (
    DECISION,
    FINDING_ID,
    HISTORICAL_TARGET,
    RULE,
...
def test_hardener_resolves_fenomeno_title_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.fenomeno.target_capture_justice.preserve_japanese_title",
        "target_vi": TARGET,
    }
...
def test_rule_is_text_data_scoped_but_covers_inheritance_prose(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    ...
    inheritance = _finding(source=f"能获得「{SOURCE_ZH}」技能折扣的因子")
    inheritance["suggested_targets_vi"] = [TARGET]
    resolved = refresh_canonical_resolutions(tmp_path, {"schema_version": 1, "findings": [inheritance]})["findings"][0]
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.fenomeno.target_capture_justice.preserve_japanese_title",
        "target_vi": TARGET,
