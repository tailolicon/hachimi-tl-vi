...
from scripts.harden_final_push_finding import (
    FINDING_ID,
    HISTORICAL_TARGET,
    RULE,
    SOURCE_JA,
...
def test_hardener_resolves_final_push_skill_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    decision = next(
        ...
        if item["decision_id"] == "audit.finding.skill-oshikiri-junbi-final-push"
    )
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.oshikiri_junbi.final_push",
        "target_vi": TARGET,
    }
...
def test_rule_does_not_overmatch_other_contexts(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    ...
    prose = _finding(source="一逃到底！她绝不会停下")
