...
from scripts.harden_frontline_target_finding import DECISION, PREFERRED, SOURCE_JA, SOURCE_ZH, TERM_ID, harden

FINDING_ID = "cf-e3ab912489ddc5f5"
...
def test_frontline_target_resolves_live_skill_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Tiến lên" in rule["forbidden"]
...
def test_frontline_target_rule_does_not_resolve_other_category(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    finding = refresh_canonical_resolutions(
        tmp_path,
...
def test_frontline_target_rule_does_not_resolve_other_source_path(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    finding = refresh_canonical_resolutions(
        tmp_path,
