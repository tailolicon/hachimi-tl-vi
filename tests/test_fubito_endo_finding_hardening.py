...
from scripts.harden_fubito_endo_finding import DECISION, TERM, harden
...
def _finding(prefix: str, *, source_path: str = "text_data_dict.json") -> dict:
    return {
        "finding_id": "cf-6ff2aa5d2b9f58ce",
        "status": "open",
        "source_zh_cn": "遠藤フビト",
        "match_mode": "contains",
...
def test_hardener_resolves_fubito_endo_credit_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert decision["target_vi"] == "Fubito Endo"
