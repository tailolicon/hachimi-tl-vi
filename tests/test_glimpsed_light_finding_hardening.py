...
from scripts.harden_glimpsed_light_finding import (
    DECISION_ID,
    FINDING_ID,
    RULE_ID,
    SOURCE_JA,
...
def _finding(prefix: str = "147", source_path: str = "text_data_dict.json") -> dict:
    return {
...
def test_glimpsed_light_hardener_resolves_jp_backed_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert rule["forbidden"] == ["Tia bình minh"]
...
def test_glimpsed_light_rule_stays_in_skill_title_scope(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
