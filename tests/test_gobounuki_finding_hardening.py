...
from scripts.harden_gobounuki_finding import FINDING_ID, RULE, SOURCE_JA, SOURCE_ZH, TARGET, harden
...
def test_hardener_resolves_category147_skill_and_is_idempotent(tmp_path: Path) -> None:
    ...
    assert "Vọt thẳng lên" in rule["forbidden"]
    ...
    decision = next(
        ...
        if item["decision_id"] == "audit.finding.skill-gobounuki"
    )
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.gobounuki.consecutive_overtake",
        "target_vi": TARGET,
    }
    assert resolved["review_resolution"] == {
        "decision_id": "audit.finding.skill-gobounuki",
        "action": "lock",
        "target_vi": TARGET,
    }
...
def test_rule_does_not_overmatch_non_skill_context(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    ...
    prose = _finding(source="她一跃而上冲到前排")
