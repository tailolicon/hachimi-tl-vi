...
from scripts.harden_hirameki_landing_finding import (
    FINDING_ID,
    HISTORICAL_TARGET,
    RULE,
    SOURCE_JA,
...
def test_hardener_resolves_hirameki_landing_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    decision = next(item for item in reviews["decisions"] if item["decision_id"] == "audit.finding.skill-hirameki-landing")
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.hirameki_landing.preserve_japanese_title",
        "target_vi": TARGET,
    }
...
def test_rule_preserves_star_and_does_not_overmatch(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    assert "☆" in TARGET
    ...
    prose = _finding(source="闪光☆着陆的继承")
