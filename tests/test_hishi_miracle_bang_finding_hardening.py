...
from scripts.harden_hishi_miracle_bang_finding import (
    DECISION_ID,
    FINDING_ID,
    RULE_ID,
    SOURCE_JA,
...
def test_hardener_resolves_hishi_miracle_unique_skill_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Bùm☆Kỳ tích giáng trần!" in rule["forbidden"]
...
def test_contains_rule_covers_inheritance_text_but_not_other_file(tmp_path: Path) -> None:
    ...
    longer = _finding(source=f"力量上限与根性上限提升，能获得「{SOURCE_ZH}」技能灵感的因子")
