...
def test_ichibanboshi_resolves_song_title_table(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_ichibanboshi_does_not_resolve_outside_song_table(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    ledger = {"schema_version": 1, "findings": [{
        "finding_id": "cf-test-ichibanboshi-wrong", "status": "open", "source_zh_cn": "イチバン星が駆ける空",
        "match_mode": "exact", "source_paths": ["text_data_dict.json"], "key_exact": [],
