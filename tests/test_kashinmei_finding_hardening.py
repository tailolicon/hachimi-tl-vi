...
from scripts.harden_kashinmei_finding import FINDING_ID, RULE, SOURCE_JA, SOURCE_ZH, TARGET, harden
...
def test_hardener_resolves_live_finding_shape_and_is_idempotent(tmp_path: Path) -> None:
    ...
    decision = next(item for item in reviews["decisions"] if item["decision_id"] == "audit.finding.skill-kashinmei")
