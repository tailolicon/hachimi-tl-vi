...
def test_keppare_kagayaki_straight_resolves_live_finding_scope(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_keppare_kagayaki_straight_does_not_resolve_outside_text_data(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    ledger = {"schema_version": 1, "findings": [{
