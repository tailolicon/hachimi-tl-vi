...
from scripts.harden_kiseki_omoi_kanade_finding import FINDING_ID, RULE, SOURCE_JA, SOURCE_ZH, TARGET, harden
...
def test_hardener_resolves_live_finding_shape_and_is_idempotent(tmp_path: Path) -> None:
    ...
    assert "Tâm ý·Tấu vang·Kỳ tích" in rule["forbidden"]
    ...
    decision = next(
        ...
        if item["decision_id"] == "audit.finding.skill-kiseki-omoi-kanade"
    )
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.ks_miracle.kiseki_omoi_kanade",
        "target_vi": TARGET,
    }
