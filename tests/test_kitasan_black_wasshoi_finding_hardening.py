...
from scripts.harden_kitasan_black_wasshoi_finding import DECISION, PREFERRED, SOURCE_JA, SOURCE_ZH, TERM_ID, harden

FINDING_ID = "cf-64aebd49fa203b6b"
...
def test_kitasan_black_wasshoi_resolves_live_skill_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_kitasan_black_wasshoi_rule_is_category_and_path_scoped(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
