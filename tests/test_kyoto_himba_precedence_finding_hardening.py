...
def _seed(tmp_path: Path) -> None:
    ...
    _write(
        glossary / "ui_community_terms.json",
        {
            "schema_version": 1,
            "terms": [{
                "id": COMPONENT_TERM_ID,
                "category": "race",
                "source_aliases": ["赛马娘锦标"],
                "preferred": "Uma Musume Stakes",
                "accepted": ["Uma Musume Stakes"],
    ...
    _write(
        glossary / "terminology_reviews.json",
        {
            "schema_version": 1,
            "decisions": [{
                "decision_id": COMPONENT_DECISION_ID,
                "source_zh_cn": "赛马娘锦标",
                "action": "lock",
                "target_vi": "Uma Musume Stakes",
                "source_paths": ["text_data_dict.json"],
                "json_path_prefixes": [],
                "match_mode": "contains",
            }],
...
def test_full_kyoto_race_wins_over_component_and_hardener_is_idempotent(tmp_path: Path) -> None:
    ...
    assert finding["review_resolution"]["target_vi"] != "Uma Musume Stakes"
