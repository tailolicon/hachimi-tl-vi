...
from scripts.harden_loh_league_score_alias_finding import (
    ALIAS,
    BASE_TERM_ID,
    BRIDGE_TERM_ID,
    DECISION_ID,
...
def _finding(source_path: str = "localize_dict.json") -> dict:
    return {
        "finding_id": "cf-test-loh-league-score-alias",
        "status": "open",
        "source_zh_cn": ALIAS,
        "match_mode": "contains",
        ...
        "json_path_prefixes": [],
        "kinds": ["terminology"],
        "concepts": ["League of Heroes League Score alias"],
        "suggested_targets_vi": [],
        "confidence_levels": ["high"],
        "reasons": ["Recurring League of Heroes score alias."],
        "evidence_count": 1,
        "evidence": [],
        ...
        "terms": [
            {
                ...
                "category": "mechanic",
                "key_prefixes": ["Heroes"],
                "source_aliases": ["リーグスコア", "联赛得分"],
                "preferred": "League Score",
                "compact": [],
                "accepted": ["League Score"],
                "forbidden": ["Điểm giải đấu"],
                "require_accepted": True,
                "basis": "Named score used by League of Heroes UI.",
            }
        ],
    }
...
def test_hardener_review_locks_bridge_and_resolves_live_scope(tmp_path: Path) -> None:
    ...
    assert decision["target_vi"] == "League Score"
