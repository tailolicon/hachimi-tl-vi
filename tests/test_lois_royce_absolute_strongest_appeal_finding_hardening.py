...
from scripts.harden_lois_royce_absolute_strongest_appeal_finding import (
    DECISION_ID,
    FINDING_ID,
    RULE_ID,
    SOURCE_JA,
...
def test_contains_rule_covers_longer_inheritance_text_but_not_other_file(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True

    longer = _finding(source=f"力量上限与智力上限提升，能获得「{SOURCE_ZH}」技能灵感的因子")
