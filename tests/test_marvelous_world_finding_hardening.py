...
from scripts.harden_marvelous_world_finding import (
    DECISION,
    FINDING_ID,
    PREFERRED,
    SOURCE_JA,
...
def _finding(*, prefix: str = "147") -> dict[str, object]:
    return {
        ...
        "review_resolution": {
            "decision_id": "parallel.ctx-67f8551f77807292-v1.term-0009.05",
            "action": "defer",
            "target_vi": None,
        },
    }
...
def test_marvelous_world_resolves_bridge_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Vạn sắc☆Thế giới★Tươi đẹp" in rule["forbidden"]
...
def test_marvelous_world_rule_does_not_resolve_other_category(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    refreshed = refresh_canonical_resolutions(
        tmp_path,
        {"schema_version": 1, "findings": [_finding(prefix="16")]},
    )
    finding = refreshed["findings"][0]
