...
from scripts.harden_mission_triumph_finding import (
    FINDING_ID,
    HISTORICAL_TARGET,
    RULE,
    SOURCE_JA,
...
def test_hardener_resolves_mission_triumph_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    decision = next(item for item in reviews["decisions"] if item["decision_id"] == "audit.finding.skill-mission-triumph")
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.mission_triumph.unique",
        "target_vi": TARGET,
    }
...
def test_rule_is_scoped_to_text_data_and_preserves_exact_display_title(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    assert TARGET == SOURCE_JA
    assert TARGET != HISTORICAL_TARGET

    wrong_path = _finding(source_path="localize_dict.json")
    wrong_path["suggested_targets_vi"] = [TARGET]
    resolved = refresh_canonical_resolutions(tmp_path, {"schema_version": 1, "findings": [wrong_path]})
