...
from scripts.harden_north_flight_arrow_line_finding import DECISION, PREFERRED, SOURCE_JA, SOURCE_ZH, TERM_ID, harden

FINDING_ID = "cf-c78e8d766172c136"
...
def test_north_flight_arrow_line_resolves_live_skill_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_north_flight_arrow_line_rule_is_category_and_path_scoped(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
