...
from scripts.harden_peep_peep_pow_finding import DECISION_ID, FINDING_ID, RULE_ID, SOURCE_JA, SOURCE_ZH, TARGET, harden
...
def test_hardener_resolves_peep_peep_pow_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert "Xem tôi có đánh cho bạn hoa mắt không nào!" in rule["forbidden"]
