...
from scripts.harden_que_alegria_finding import FINDING_ID, RULE, SOURCE_ZH, TARGET, harden
...
def test_hardener_resolves_live_finding_shape_and_is_idempotent(tmp_path: Path) -> None:
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.que_alegria",
        "target_vi": TARGET,
    }
    assert resolved["review_resolution"] == {
        "decision_id": "audit.finding.skill-que-alegria",
        "action": "lock",
        "target_vi": TARGET,
    }
