...
from scripts.harden_ranzen_kappo_finding import (
    DECISION,
    FINDING_ID,
    HISTORICAL_TARGETS,
    SOURCE_JA,
...
def _finding(*, source_path: str = "text_data_dict.json", source: str = SOURCE_ZH) -> dict:
    return {
...
def test_hardener_resolves_ranzen_kappo_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_rule_is_exact_and_does_not_escape_source_path(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True

    # The category-172 inheritance text contains the title but is not itself
    # the Skill title. Exact matching must leave such a broader finding open.
    inherited_prose = _finding(source=f"速度上限提升，能获得「{SOURCE_ZH}」技能折扣")
    inherited_prose["suggested_targets_vi"] = [TARGET]
    resolved = refresh_canonical_resolutions(
        tmp_path, {"schema_version": 1, "findings": [inherited_prose]}
    )["findings"][0]
