...
from scripts.harden_sake_sake_watashi_finding import (
    DECISION,
    FINDING_ID,
    HISTORICAL_TARGET,
    RULE,
...
def test_hardener_resolves_sake_sake_watashi_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.sake_sake_watashi.preserve_japanese_title",
        "target_vi": TARGET,
    }
...
def test_rule_is_scoped_to_text_data_path(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
