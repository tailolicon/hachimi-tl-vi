...
from scripts.harden_sazameki_arrow_finding import FINDING_ID, RULE, SOURCE_JA, SOURCE_ZH, TARGET, harden
...
def test_hardener_resolves_live_finding_shape_and_is_idempotent(tmp_path: Path) -> None:
    ...
    decision = next(
        ...
        if item["decision_id"] == "audit.finding.skill-sazameki-arrow"
    )
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.sazameki_arrow",
        "target_vi": TARGET,
    }
    assert resolved["review_resolution"] == {
        "decision_id": "audit.finding.skill-sazameki-arrow",
        "action": "lock",
        "target_vi": TARGET,
    }
