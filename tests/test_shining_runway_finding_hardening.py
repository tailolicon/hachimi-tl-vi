...
from scripts.harden_shining_runway_finding import (
    DECISION,
    FINDING_ID,
    HISTORICAL_TARGETS,
    RULE,
...
def test_hardener_resolves_shining_runway_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert all(item in rule["forbidden"] for item in HISTORICAL_TARGETS)
...
def test_rule_requires_exact_source_and_text_data_path(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    ...
    assert (
        refresh_canonical_resolutions(tmp_path, {"schema_version": 1, "findings": [wrong_path]})["findings"][0][
            "canonical_resolution"
        ]
        is None
    )

    prose = _finding(source=f"{SOURCE_ZH}的继承")
    prose["suggested_targets_vi"] = [TARGET]
    assert (
        refresh_canonical_resolutions(tmp_path, {"schema_version": 1, "findings": [prose]})["findings"][0][
            "canonical_resolution"
        ]
        is None
    )
