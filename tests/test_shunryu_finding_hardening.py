...
from scripts.harden_shunryu_finding import DECISION, TERM, harden
...
def _finding(*, source_path: str = "text_data_dict.json") -> dict:
    return {
        "finding_id": "cf-74b484f15fb579a7",
        "status": "open",
        "source_zh_cn": "俊龍",
        "match_mode": "contains",
        ...
        "review_resolution": {"decision_id": "parallel.ctx-old.shunryu-defer", "action": "defer", "target_vi": None},
    }


def test_hardener_resolves_shunryu_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    assert harden(tmp_path) is False
    community = json.loads((tmp_path / "glossary" / "ui_community_terms.json").read_text(encoding="utf-8"))
    rule = next(item for item in community["terms"] if item["id"] == TERM["id"])
    assert rule["preferred"] == "Shunryu"
    assert rule["source_paths"] == ["text_data_dict.json"]
    assert "俊龍" in rule["forbidden"]
    reviews = json.loads((tmp_path / "glossary" / "terminology_reviews.json").read_text(encoding="utf-8"))
    decision = next(item for item in reviews["decisions"] if item["decision_id"] == DECISION["decision_id"])
    assert decision["target_vi"] == "Shunryu"
