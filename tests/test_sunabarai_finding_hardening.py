...
from scripts.harden_sunabarai_finding import FINDING_ID, RULE, SOURCE_JA, SOURCE_ZH, TARGET, harden
...
def test_hardener_resolves_category147_skill_and_is_idempotent(tmp_path: Path) -> None:
    ...
    assert "Tẩy trần" in rule["forbidden"]
    ...
    decision = next(
        ...
        if item["decision_id"] == "audit.finding.skill-sunabarai"
    )
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.sunabarai.dust_off",
        "target_vi": TARGET,
    }
...
def test_rule_does_not_overmatch_prose_or_other_categories(tmp_path: Path) -> None:
    ...
    prose = _finding(source="比赛结束后先洗尘再回宿舍")
