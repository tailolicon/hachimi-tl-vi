...
from scripts.harden_supreme_be_finding import DECISION, PREFERRED, SOURCE_JA, SOURCE_ZH, TERM_ID, harden

FINDING_ID = "cf-0d296ca93e574cff"
...
def test_supreme_be_resolves_live_skill_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_supreme_be_rule_is_category_and_path_scoped(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
