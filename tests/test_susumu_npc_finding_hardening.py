...
from scripts.harden_susumu_npc_finding import DECISION, SOURCE, harden
...
def test_hardener_is_idempotent_and_item_scoped_ignore_clears_finding(tmp_path: Path) -> None:
    ...
    _write(
        tmp_path / "glossary" / "canonical_findings.json",
        {
            ...
            "findings": [
                {
                    "finding_id": "cf-0ed1b3254699aa40",
                    "status": "open",
                    "source_zh_cn": SOURCE,
                    "match_mode": "exact",
    ...
    assert DECISION["json_path_prefixes"] == [["152", "141"]]
