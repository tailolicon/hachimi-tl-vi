...
from scripts.harden_tanino_gimlet_aufheben_finding import DECISION, PREFERRED, SOURCE_JA, SOURCE_ZH, TERM_ID, harden

FINDING_ID = "cf-1e27154bb898948c"
...
def test_tanino_gimlet_aufheben_resolves_live_skill_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_tanino_gimlet_aufheben_rule_is_category_and_path_scoped(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
