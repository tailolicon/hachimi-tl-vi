...
from scripts.harden_tucker_bryne_finding import (
    DECISION_ID,
    FINDING_ID,
    RULE_ID,
    SOURCE_JA,
...
def _finding(*, prefix: list[list[str]] | None = None) -> dict[str, object]:
    return {
        ...
        "key_exact": [],
        "json_path_prefixes": prefix if prefix is not None else [["120"]],
...
def test_tucker_bryne_resolves_live_finding_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_tucker_bryne_rule_does_not_escape_scenario120(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    payload = refresh_canonical_resolutions(
        tmp_path,
        {"schema_version": 1, "findings": [_finding(prefix=[["121"]])]},
    )
    finding = payload["findings"][0]
