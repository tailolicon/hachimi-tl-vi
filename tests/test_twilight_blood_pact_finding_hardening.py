...
from scripts.harden_twilight_blood_pact_finding import (
    FINDING_IDS,
    HISTORICAL_TARGET,
    RULE,
    SOURCE_JA,
...
def test_hardener_resolves_duplicate_live_scopes_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    decision = next(
        ...
        if item["decision_id"] == "audit.finding.skill-twilight-blood-pact"
    )
    ...
    for resolved in resolved_ledger["findings"]:
        ...
        assert resolved["canonical_resolution"] == {
            "layer": "community",
            "term_id": "skill.jungle_pocket.twilight_blood_pact",
            "target_vi": TARGET,
        }
...
def test_exact_rule_does_not_overmatch_longer_source_or_other_file(tmp_path: Path) -> None:
    ...
    longer = _finding(FINDING_IDS[1], source=f"前缀{SOURCE_ZH}后缀")
    ...
    outside = _finding(FINDING_IDS[1], source_path="localize_dict.json")
