import json
from pathlib import Path

from scripts.canonical_findings import refresh_canonical_resolutions
from scripts.harden_uma_like_localize_alias_finding import (
    ALIAS,
    BASE_TERM_ID,
    BRIDGE_TERM_ID,
    DECISION_ID,
    harden,
)


def _write(path: Path, payload: dict) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def _finding(source_path: str = "localize_dict.json") -> dict:
    return {
        "finding_id": "cf-b56eee53b345a2bd",
        "status": "open",
        "source_zh_cn": ALIAS,
        "match_mode": "contains",
        "source_paths": [source_path],
        "key_exact": [],
        "json_path_prefixes": [],
        "kinds": ["terminology"],
        "concepts": ["Uma Like! action outside League of Heroes"],
        "suggested_targets_vi": ["Uma Like!"],
        "confidence_levels": ["high"],
        "reasons": ["Named feature alias must remain Uma Like! outside League of Heroes."],
        "evidence_count": 2,
        "evidence": [],
        "canonical_resolution": None,
        "review_resolution": None,
    }


def _community() -> dict:
    return {
        "schema_version": 1,
        "terms": [
            {
                "id": BASE_TERM_ID,
                "category": "mechanic",
                "key_prefixes": ["Heroes"],
                "source_aliases": ["ウマいね！", ALIAS],
                "preferred": "Uma Like!",
                "compact": [],
                "accepted": ["Uma Like!"],
                "forbidden": [],
                "require_accepted": True,
                "basis": "Player-facing feature name.",
            }
        ],
    }


def _seed(tmp_path: Path) -> Path:
    glossary = tmp_path / "glossary"
    _write(glossary / "ui_community_terms.json", _community())
    _write(glossary / "terminology_reviews.json", {"schema_version": 1, "decisions": []})
    _write(glossary / "term_registry.json", {"schema_version": 1, "terms": []})
    _write(glossary / "source_bridge_terms.json", {"schema_version": 1, "terms": []})
    return glossary


def test_hardener_preserves_heroes_term_and_resolves_localize_finding(tmp_path: Path) -> None:
    glossary = _seed(tmp_path)

    assert harden(tmp_path) is True
    assert harden(tmp_path) is False

    community = json.loads((glossary / "ui_community_terms.json").read_text(encoding="utf-8"))
    base = next(item for item in community["terms"] if item["id"] == BASE_TERM_ID)
    bridge = next(item for item in community["terms"] if item["id"] == BRIDGE_TERM_ID)
    assert base["key_prefixes"] == ["Heroes"]
    assert bridge["source_aliases"] == [ALIAS]
    assert bridge["source_paths"] == ["localize_dict.json"]
    assert bridge["match_mode"] == "contains"

    reviews = json.loads((glossary / "terminology_reviews.json").read_text(encoding="utf-8"))
    decision = next(item for item in reviews["decisions"] if item["decision_id"] == DECISION_ID)
    assert decision["action"] == "lock"
    assert decision["target_vi"] == "Uma Like!"

    finding = refresh_canonical_resolutions(
        tmp_path,
        {"schema_version": 1, "policy": {"canonical": False}, "findings": [_finding()]},
    )["findings"][0]
    assert finding["review_resolution"] == {
        "decision_id": DECISION_ID,
        "action": "lock",
        "target_vi": "Uma Like!",
    }
    assert finding["canonical_resolution"] == {
        "layer": "community",
        "term_id": BRIDGE_TERM_ID,
        "target_vi": "Uma Like!",
    }


def test_reviewed_alias_does_not_canonicalize_outside_localize_source(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    finding = refresh_canonical_resolutions(
        tmp_path,
        {"schema_version": 1, "policy": {"canonical": False}, "findings": [_finding("text_data_dict.json")]},
    )["findings"][0]
    assert finding["review_resolution"]["action"] == "lock"
    assert finding["canonical_resolution"] is None
