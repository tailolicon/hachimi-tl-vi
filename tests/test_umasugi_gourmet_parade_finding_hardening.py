...
from scripts.harden_umasugi_gourmet_parade_finding import (
    DECISION,
    FINDING_ID,
    HISTORICAL_TARGET,
    RULE,
...
def test_hardener_resolves_song_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_rule_does_not_resolve_wrong_path_or_nonexact_source(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    ...
    prose = _finding(source=f"{SOURCE} (Game Size)")
