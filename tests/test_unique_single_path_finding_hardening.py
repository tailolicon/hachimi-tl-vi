...
from scripts.harden_unique_single_path_finding import (
    FINDING_ID,
    HISTORICAL_TARGET,
    RULE,
    SOURCE_JA,
...
def test_hardener_resolves_unique_single_path_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    decision = next(item for item in reviews["decisions"] if item["decision_id"] == "audit.finding.skill-unique-single-path")
    ...
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.muji_musan_naru_ichijo_no_michi.unique_single_path",
        "target_vi": TARGET,
    }
...
def test_rule_is_exact_and_does_not_reuse_zh_wordplay(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True
    ...
    prose = _finding(source="一线生路无二亦无三的传承")
