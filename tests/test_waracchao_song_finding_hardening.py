...
from scripts.harden_waracchao_song_finding import FINDING_ID, RULE, SOURCE, TARGET, harden


def _finding(*, source: str = SOURCE, source_path: str = "text_data_dict.json", evidence_category: str = "16") -> dict:
    return {
        ...
        "evidence": [
            {
                ...
                "json_path": [evidence_category, "1073"],
                "source_text": SOURCE,
                "target_text": "Cùng cười nào!",
            }
        ],
    }
...
def test_waracchao_repairs_scope_and_resolves_song_title(tmp_path: Path) -> None:
    _seed(tmp_path)
...
def test_waracchao_scope_repair_requires_category_16_evidence(tmp_path: Path) -> None:
    _seed(tmp_path)
    ledger_path = tmp_path / "glossary" / "canonical_findings.json"
    ledger = json.loads(ledger_path.read_text(encoding="utf-8"))
    ledger["findings"] = [_finding(evidence_category="181")]
    ledger_path.write_text(json.dumps(ledger), encoding="utf-8")

    assert harden(tmp_path) is True
    repaired = json.loads(ledger_path.read_text(encoding="utf-8"))["findings"][0]
    assert repaired["json_path_prefixes"] == []
    resolved = refresh_canonical_resolutions(
        tmp_path, {"schema_version": 1, "findings": [repaired]}
    )["findings"][0]
