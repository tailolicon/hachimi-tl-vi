...
def test_hardener_locks_verified_jp_identity_and_resolves_finding(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert rule["preferred"] == SOURCE_JA == TARGET
    assert rule["accepted"] == [SOURCE_JA]
    ...
    assert decision["target_vi"] == SOURCE_JA
    ...
    assert resolved["suggested_targets_vi"] == [SOURCE_JA]
    assert resolved["canonical_resolution"] == {
        "layer": "community",
        "term_id": "skill.wonder_acute.yumori_no_wagokoro",
        "target_vi": SOURCE_JA,
    }
...
def test_exact_jp_only_skill_lock_does_not_overmatch_alias_or_other_file(tmp_path: Path) -> None:
    _seed(tmp_path)
    assert harden(tmp_path) is True

    longer = _finding(source=f"前缀{SOURCE_ZH}后缀", match_mode="contains")
