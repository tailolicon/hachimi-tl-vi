...
from scripts.harden_yume_no_kotae_finding import DECISION_ID, FINDING_ID, RULE_ID, SOURCE, TARGET, harden


def _finding(source: str = SOURCE, source_path: str = "text_data_dict.json") -> dict:
    return {
...
def test_hardener_resolves_yume_no_kotae_and_is_idempotent(tmp_path: Path) -> None:
    _seed(tmp_path)
    ...
    assert SOURCE in rule["forbidden"]
    assert "Lời đáp của giấc mơ" in rule["forbidden"]
...
def test_exact_rule_does_not_cover_longer_text_or_other_file(tmp_path: Path) -> None:
    ...
    longer = _finding(source=f"楽曲『{SOURCE}』を獲得")
